...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate the paper-muncher benchmark ledger")
    parser.add_argument("--size", required=True, type=int)
    parser.add_argument("--output", required=True)
    args = parser.parse_args()

    if args.size < 100 or args.size % 100:
        parser.error("--size must be a positive multiple of 100")

    with open(args.output, "w", encoding="utf-8") as output:
        render(args.size, output)
